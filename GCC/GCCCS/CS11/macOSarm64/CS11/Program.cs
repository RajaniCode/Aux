...
// 'target-typed object creation' // CS 9.0+
...
for (int i = 0; i < 25; i++)
{ 
    Console.Write($"{++implementationOperator}" + ", ");
}
Console.Write($"{++implementationOperator}");	
Console.WriteLine();
Console.WriteLine("overloading unary operator --");
...
/*************************************************************************************************************************/
Console.WriteLine("CS 11.0, File-local keyword 'file' is type modifier");
Console.WriteLine("CS 11.0, File-local cannot use access modifier");
Console.WriteLine("CS 11.0, File-local default access modifier is internal");
...
record RecordOperatorOverloading
{
    ...
    // unchecked context
    public static RecordOperatorOverloading operator +(RecordOperatorOverloading operator1, RecordOperatorOverloading operator2)
    {
    ...
    public static RecordOperatorOverloading operator checked +(RecordOperatorOverloading operator1, RecordOperatorOverloading operator2)
    {
        checked
        {
            RecordOperatorOverloading operatorOverload = new();
...
file class FileLocalAttributeClient
{
    public void Print()
    {
        Console.WriteLine("CS 11.0, File-local classes can be attribute types");
        Console.WriteLine("CS 11.0, File-local can be used as attributes within file-local types");
...
class AttributeClient
{
    public void Print()
    {
        Console.WriteLine("CS 11.0, File-local can be used as attributes within non-file-local types");
...
class GenericConstarint<T> where T : new()
{
    ...
    public void Print()
    {
        Console.WriteLine("T t GetType(): " + t?.GetType());
        Console.WriteLine("T t GetType() Name: " + t?.GetType().Name);
        Console.WriteLine("typeof(T): " + typeof(T));
        Console.WriteLine("typeof(T) Name: " + typeof(T).Name);
        Console.WriteLine($"T t GetT(): {GetT()}");

        // 'RquiredMembers' cannot satisfy the 'new()' constraint on parameter 'T' in the generic type or or method 'GenericConstarint<T>' because the class 'RquiredMembers' has required members.
...
readonly struct AutoDefaultStructure
{
    ...
    // Prior to CS 11.0, a constructor of a structure type must initialize all instance fields of the type.
    // Prior to CS 10.0, you can't declare a parameterless constructor.
    // Prior to CS 10.0, you can't initialize an instance field or property at its declaration.
    // Auto-default struct
    ...
    // Pre-CS 11.0 // Error CS0843: Auto-implemented property 'AutoDefaultStructure.AutoImplementedProperty' must be fully assigned before control is returned to the caller
...
struct StructurePreCS11
{
    ...
    // Pre-CS 11.0 // Error CS0188: The 'this' object cannot be used before all of its fields have been assigned
...
class NameOfExpression
{
    ...
    {
        // 'local function attributes' // CS 9.0+
        ...
        // 'lambda attributes' // CS 10.0+
        // 'inferred delegate type' // CS 10.0+
...
struct Point3D
{
    ...
    public ref readonly Point3D Origin => ref origin; // CS 7.0 property ref return of static member
}
// CS 11.0 // ref fields
...
{
    // ref field 
    static string staticString = "ref field assignment at declaration with static member only in CS 11.0";
    // readonly ref struct
    // readonly ref string refString = ref staticString; // Not to be confused with CS 7.0 property ref return of static member
...
class ImprovedMethodGroupConversionToDelegate
{
    ...
    public void Print()
    {
        Console.WriteLine("CS 11.0: The compiler may cache the delegate object created from a method group.");
        Console.WriteLine("Pre-CS 11.0: Use a lambda expression to reuse a single delegate object.");

        System.Diagnostics.Stopwatch stopwatch = System.Diagnostics.Stopwatch.StartNew();
        for (int i =0; i < 1000000; i++ )
        {
            // CS 11.0 .NET 7 // Compare this with CS 10.0 .NET 6
...
dotnet-sdk-7.0.410-osx-arm64.pkg
...
5. Newlines in string interpolation expressions
Academic grading for 93 is Grade: A, GPA: 4.0.
...
7. File-local types
CS 11.0, File-local keyword 'file' is type modifier
CS 11.0, File-local cannot use access modifier
CS 11.0, File-local default access modifier is internal
Method in FileB
Method in FileC
CS 11.0, File-local classes can be attribute types
CS 11.0, File-local can be used as attributes within file-local types
[<Program>F540A75E9526D9D6EB1A1A8F6D072833DFCC9CC1B5477D6190AFEFAF7E178372F__FileLocalAttribute()]
CS 11.0, File-local can be used as attributes within non-file-local types
[<Program>F540A75E9526D9D6EB1A1A8F6D072833DFCC9CC1B5477D6190AFEFAF7E178372F__FileLocalAttribute()]
...
14. Improved method group conversion to delegate
CS 11.0: The compiler may cache the delegate object created from a method group.
Pre-CS 11.0: Use a lambda expression to reuse a single delegate object.
Elapsed Time: 00:00:00.0069449
Elapsed Time Lambda: 00:00:00.0062320
